...
std::tuple<std::vector<float>,std::vector<float>, std::vector<float>, std::vector<float>, std::vector<float>> methodRungeKuttaDiff() {
    DynamicModel dynamicModel;
    PID pid_u(30.5, 32.2, 2.6); // Adjusted PID for speed
    PID pid_psi(9.0, 0.5, 34.1); // Adjusted PID for heading
    ...
    float setpoint_u = 0.1; // target speed (m/s)
    ...
    for (int ii = 0; ii < 1000; ii++) {
        ...
        setpoint_psi = 0.25 * sin(0.02 * ii);

        // compute control inputs
        float control_u = pid_u.calculate(setpoint_u, upsilon(0), dt);
        float control_psi = pid_psi.calculate(setpoint_psi, upsilon(2), dt);
...
int main() {
   ...
   // plot2D(time, xX, "PID: X Position vs Time", "Time (s)", "X Position (m)");
   // plot2D(time, yY, "PID: Y Position vs Time", "Time (s)", "Y Position (m)");
    plot2D2D(time, psi, psi_desired, "ASV: Heading (Psi) vs Time", "Time (s)", "Heading (rad)");
}
