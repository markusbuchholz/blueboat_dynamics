...
// g++ mpc_blue_dyn_with_disturbances.cpp -o mpd_dyn_dist -I/usr/include/eigen3 -I/usr/include/python3.12 -I/usr/lib/python3/dist-packages/numpy/core/include -lpython3.12
...
const float Lambda = 25000.0;   // Wave length
...
public:
    ...
    Vector3f function_1(Vector3f upsilon, float time) {
        ...
        // Total disturbances
        float Fx = T(0) - (Xu + Xuu * abs(upsilon(0))) + F_wave_x + F_wind_x + current_velocity_x;
        float Fy = T(1) - (Yv * upsilon(1) + Yr * upsilon(2)) + F_wave_y + F_wind_y + current_velocity_y;

        // Acceleration vector [u' v' r']
        upsilon_dot = M.inverse() * (Vector3f(Fx, Fy, T(2)) - (C * upsilon) - (D * upsilon) + Delta);
...
std::tuple<std::vector<float>, std::vector<float>, std::vector<float>, std::vector<float>, std::vector<float>> methodRungeKuttaMPC() {
    ...
    float setpoint_u = 0.5;    // target speed (m/s)
    ...
    for (int ii = 0; ii < 1000; ii++) {
        ...
        for (int jj = 1; jj <= horizon; jj++) {
            predicted_states[jj] = predicted_states[jj - 1] + 0.1 * setpoint_u;  // predicting future velocity states
            predicted_psi_states[jj] = predicted_psi_states[jj - 1] + 0.1 * setpoint_psi;  // predicting future yaw states
        }
...
//---------------------------------------------------------------------------------------------------------
void plot2D2D(std::vector<float> xX, std::vector<float> yY1, std::vector<float> yY2, const std::string &title, const std::string &xlabel, const std::string &ylabel) {
    plt::title(title);
    plt::named_plot("solution yaw", xX, yY1);
    plt::named_plot("desired yaw", xX, yY2);
    plt::xlabel(xlabel);
    plt::ylabel(ylabel);
    plt::legend();
    plt::show();
...
int main() {
    ...
    auto time = std::get<4>(vdp);

    //plot2D(time, xX, "MPC: X Position vs Time", "Time (s)", "X Position (m)");
    //plot2D(time, yY, "MPC: Y Position vs Time", "Time (s)", "Y Position (m)");
    //plot2D(time, psi, "MPC: Heading (Psi) vs Time", "Time (s)", "Heading (rad)");
    plot2D2D(time, psi, psi_desired, "MPC ASV with distrubances (waves:3m, wind 5m/s, currents:2.5m/s): Heading (Psi) vs Time", "Time (s)", "Heading (rad)");
}
