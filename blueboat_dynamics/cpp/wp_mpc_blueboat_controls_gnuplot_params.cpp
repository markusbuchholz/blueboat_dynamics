...
// g++ wp_mpc_blueboat_controls_gnuplot_params.cpp -o t -I/usr/include/eigen3 -I/usr/include/boost -lboost_system -lboost_filesystem -lboost_iostreams
...
public:
    ...
    float Vc;           // Speed of the ocean current (m/s)

    DynamicModel(float A, float Vw, float Vc ) : A(A), Vw(Vw), Vc(Vc) {
        upsilon << 0, 0, 0;
...
methodRungeKuttaMPC(const vector<Vector2f> &waypoints, float &setpoint_u, float &A, float &Vx, float &Vc) {


    DynamicModel dynamicModel (A, Vx, Vc);
...
void writeMultiRunDataFile(const std::string &filename, const std::vector<float> &x, const std::vector<float> &y1, const std::vector<float> &y2, const std::vector<float> &y3, const std::vector<float> &y4) {
...
//---------------------------------------------------------------------------------------------------------
void plotWithGnuplotMultiple(const std::string &dataFile, const std::string &title, const std::string &xlabel, const std::string &ylabel,
                             const std::string &y1LabelRun1, const std::string &y2LabelRun1,
                             const std::string &y1LabelRun2, const std::string &y2LabelRun2) {
    std::string gnuplotCommand = 
        "gnuplot -e \"set title '" + title + 
        "'; set xlabel '" + xlabel + 
        "'; set ylabel '" + ylabel + 
        "'; set style line 1 lt rgb '#1f77b4' lw 1; " // blue solid for Run 1
        "set style line 2 lt rgb '#2ca02c' lw 1; "    // orange solid for Run 1
        "set style line 3 lt rgb '#1f77b4' dashtype 2 lw 1; " // blue dashed for Run 2
        "set style line 4 lt rgb '#2ca02c' dashtype 2 lw 1; " // orange dashed for Run 2
        "plot '" + dataFile + "' using 1:2 with lines linestyle 1 title '" + y1LabelRun1 + 
        "', '" + dataFile + "' using 1:3 with lines linestyle 2 title '" + y2LabelRun1 + 
        "', '" + dataFile + "' using 1:4 with lines linestyle 3 title '" + y1LabelRun2 + 
        "', '" + dataFile + "' using 1:5 with lines linestyle 4 title '" + y2LabelRun2 + 
        "'; pause -1\"";
    system(gnuplotCommand.c_str());
}
...
int main() {
    ...
    // First run
    float setpoint_u1 = 0.2;
    float A1 = 5;
    float Vx1 = 5;
    float Vc1 = 3;

    auto vdp1 = methodRungeKuttaMPC(waypoints, setpoint_u1, A1, Vx1, Vc1);
    auto psi1 = std::get<2>(vdp1);
    auto psi_desired1 = std::get<3>(vdp1);
    auto thrustPort1 = std::get<4>(vdp1);
    auto thrustStarboard1 = std::get<5>(vdp1);
    auto time1 = std::get<6>(vdp1);

    // Second run
    float setpoint_u2 = 0.2;
    float A2 = 2.5;
    float Vx2 = 2.5;
    float Vc2 = 1.5;

    auto vdp2 = methodRungeKuttaMPC(waypoints, setpoint_u2, A2, Vx2, Vc2);
    auto psi2 = std::get<2>(vdp2);
    auto psi_desired2 = std::get<3>(vdp2);
    auto thrustPort2 = std::get<4>(vdp2);
    auto thrustStarboard2 = std::get<5>(vdp2);
    auto time2 = std::get<6>(vdp2);

   
    writeMultiRunDataFile("heading_multi_run_data.dat", time1, psi1, psi_desired1, psi2, psi_desired2);
    writeMultiRunDataFile("thrust_multi_run_data.dat", time1, thrustPort1, thrustStarboard1, thrustPort2, thrustStarboard2);

    // Plot using gnuplot (both runs on the same plot)
    plotWithGnuplotMultiple("heading_multi_run_data.dat", 
                            "MPC ASV with disturbances: Heading (Psi) vs Time (Multi-run)", 
                            "Time (s)", "Heading (rad)", 
                            "Run 1: Solution yaw", "Run 1: Desired yaw",
                            "Run 2: Solution yaw", "Run 2: Desired yaw");
    
    plotWithGnuplotMultiple("thrust_multi_run_data.dat", 
                            "Thrust MPC ASV with disturbances: (Run1: waves:5m, wind 5m/s, currents:3m/s, Run_2: waves:2.5m, wind 2.5m/s, currents:1.5m/s)", 
                            "Time (s)", "Thrust", 
                            "Run 1: ThrustPort", "Run 1: ThrustStb",
                            "Run 2: ThrustPort", "Run 2: ThrustStb");
